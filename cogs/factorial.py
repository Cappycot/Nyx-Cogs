...
def locate_numbers(string):
    ...
    its factorial.
    """
    raw_results = findall("[ ]+[0-9]*[*\/+-]?[^.!]?[0-9]+![?,. ]",
                          " {} ".format(string.replace(" ", "  ")))
    # Latest tweaks: We now ignore numbers with more than one exclamation
    # mark behind them. (e.g. 2!!) We also fixed the "X!X!" repeating issue.
    ...
    for result in raw_results:
        ...
        if len(result) <= length_limit and result:
            new_factorial = True
            # Do not duplicate factorial listings that have the same number
            # and prefix. e.g. "2!? This person only has 2!?"
            for old_result in results:
                if prefix == old_result[0] and int(result) == old_result[1]:
                    new_factorial = False
                    break
            if new_factorial:
